...
class Tools:
    ...
    async def explore_area(
        ...
        # Open-in-Maps overlay: redirect through proxy to bypass COOP
        maps_open_url = f"https://www.google.com/maps/search/?q={urllib.parse.quote(f'{cat_q} in {area}')}"
        open_url = _redirect_url(self.valves.frontend_url, maps_open_url)

        # Wrap in backend proxy so the iframe auto-resizes via postMessage
        wrapper_url = _embed_url(self.valves.frontend_url, embed_url, open_url=open_url)
